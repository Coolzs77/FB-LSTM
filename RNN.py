...
def test(model, args, test_loader, scaler, time_column, model_name,div,start_point, select=False):
    ...
    if select:
        model.load_state_dict(torch.load(fr'.\save\RNN_policy_model{division}.pth'))
    else:
        model.load_state_dict(torch.load(fr'.\save\RNN{division}.pth'))
...
if __name__ == '__main__':
    ...
    # 实例化
    model = RNN(args.input_size, args.hidden_size, args.laryer_num, args.output_size, args.pre_len).to(device)
    ...
    # torch.save(model.state_dict(), fr'.\save\RNN{division}.pth')
    #
    # train(model_policy, args, train_loader, scaler, device)
    # torch.save(model_policy.state_dict(), fr'.\save\RNN_policy_model{division}.pth')

    # 测试
    start_point=0  # 设置起点
    test(model, args, test_loader, scaler, time_column, 'RNN', args.div,start_point ,select=False)
    plot_results(args, 'RNN',division,start_point)
    plot_single_model_violin(args, 'RNN',division,start_point)

    test(model_policy, args, test_loader, scaler, time_column, 'RNN_policy', args.div,start_point,select=True)
    plot_results(args, 'RNN_policy',division,start_point)
    plot_single_model_violin(args, 'RNN_policy',division,start_point)
