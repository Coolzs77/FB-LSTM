...
def plot(y, pred, ind):
    # plot
    plt.figure(figsize=(10, 5))
    plt.plot(pred, color='red', label='true value')

    plt.plot(y, color='blue', label='pred value')
...
def test(model, args, test_loader, scaler, time_column, model_name, div,start_point,select=False):
    ...
    if select:
        model.load_state_dict(torch.load(fr'.\save\GRU_policy_model{division}.pth'))
    else:
        model.load_state_dict(torch.load(fr'.\save\GRU{division}.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model', type=str, default='GRU', help="GRU")
    parser.add_argument('-window_size', type=int, default=6, help="时间窗口大小, window_size > pre_len")
    ...
    parser.add_argument('-data_path', type=str, default=r"4G网络控制器 - 8路一号.xlsx", help="你的数据数据地址")
    parser.add_argument('-target', type=str, default=['湿度传感器一号'], help='需要预测的特征列，最后保存在csv文件里')
    parser.add_argument('-input_size', type=int, default=16, help='你的特征个数不算时间那一列')
    ...
    # div
    parser.add_argument('-div', type=float, default=0.8, help="训练集和测试集的划分比例")
    ...
    # 实例化
    model = GRU(args.input_size, args.hidden_size,  args.laryer_num, args.output_size, args.pre_len).to(device)
    ...
    # torch.save(model.state_dict(), fr'.\save\GRU{division}.pth')
    #
    # train(model_policy, args, train_loader, scaler, device)
    # torch.save(model_policy.state_dict(), fr'.\save\GRU_policy_model{division}.pth')

    # 测试
    start_point=0
    # 测试 GRU 模型（不启用选择策略）
    test(model, args, test_loader, scaler, time_column, 'GRU',args.div,start_point, select=False)
    plot_results(args, 'GRU',division,start_point)
    plot_single_model_violin(args, 'GRU',division,start_point)

    # 测试 GRU+Policy 模型（启用选择策略）

    test(model_policy, args, test_loader, scaler, time_column, 'GRU_Policy',args.div,start_point, select=True)
    plot_results(args, 'GRU_Policy',division,start_point)
    plot_single_model_violin(args, 'GRU_Policy',division,start_point)
